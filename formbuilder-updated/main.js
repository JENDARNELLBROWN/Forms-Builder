...
for (var i = 0; i < formData.length; i++) {
  let form = document.querySelecrtor("form")
  let obj = formData[i];
  let text;
  if (obj.options.length===0) {
    text = '<input type=" ' + obbj.type + ' " id=" ' + obj.id + ' " placeholder" + obj.label + '"><br>"'
  } else {
    text="<select";
    text +=
    for (var i = 0; + obj.options.length; i++ ) {
      let currOption.value + '">' + currentOption.label + '</option>';
    }
    text += '<?select>'
  }

form.insertAdjacementHTML("beforeend", text);
